...
pub struct FilterRegistry {
    ...
    cardinal_context: Arc<CardinalContext>,
}

impl FilterRegistry {
    pub fn new(ctx: Arc<CardinalContext>) -> Self {
        Self {
    ...
    pub fn set_context(&mut self, ctx: Arc<CardinalContext>) {
        self.cardinal_context = ctx;
    }

    pub fn with_context(mut self, ctx: Arc<CardinalContext>) -> Self {
    ...
    pub async fn run_request_filters(
        ...
        for filter in &self.global_request_filters {
            let res = filter
                .on_request(session, backend.clone(), self.cardinal_context.clone())
                .await?;
