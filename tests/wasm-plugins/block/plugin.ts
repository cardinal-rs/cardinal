...
export function handle(_ptr: i32, _len: i32): i32 {
    const blockFlag = getHeader("x-block");
    if (blockFlag != null && blockFlag.toLowerCase() == "true") {
        setStatus(403);
        setHeader("x-blocked", "true");
        setHeader("x-decision", "blocked");
        return 0;
    }

    setHeader("x-blocked", "false");
    setHeader("x-decision", "allowed");
    return 1;
}
